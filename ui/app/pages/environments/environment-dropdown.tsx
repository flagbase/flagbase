...
export const useUpdateActiveEnvironment = () => {
    ...
    const query = useMutation(['updateActiveEnvironment'], {
        mutationFn: async (environmentKey: string) => {
            ...
            const returnEnvironment = environments?.find((env) => env.attributes.key === environmentKey)
            return returnEnvironment
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['activeEnvironment'])
        },
    })
...
export const EnvironmentDropdown = () => {
    ...
    const activeEnvironment = environments?.find((env) => env.attributes.key === activeEnvironmentKey)
    ...
    useEffect(() => {
        if (isSuccess && activeEnvironment) {
            navigate(replaceEnvironmentKey(pathname, activeEnvironment.attributes.key))
        }
